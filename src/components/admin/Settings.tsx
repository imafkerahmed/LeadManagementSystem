...
import { Users, Shield, ArrowLeft, Check, X, Lock, Search, Loader2, Key, UserCheck } from "lucide-react";
...
export default function AdminSettings() {
  const [currentView, setCurrentView] = useState<"menu" | "users" | "access_control" | "role_policy" | "user_policy">("menu");
  ...
  const hasUserManagementAccess = () => {
    ...
    const policy = policies.find((p) => p.sectionKey === "admin_user_management");
    ...
    return !denied.includes(userId) && (allowed.includes(userId) || roles.includes(currentUserRole));
  };
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error(error);
      toast.error("Failed to load access control data: " + (error.message || String(error)));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const updatePolicyInPb = async (policyId: string, updates: Partial<PolicyRecord>) => {
    ...
    try {
      ...
      console.error(error);
      toast.error("Failed to update policy: " + (error.message || String(error)));
      await loadData();
    } finally {
      setIsSaving("");
    }
  ...
  const handleToggleRole = async (policy: PolicyRecord, role: string) => {
    ...
    setPolicies(policies.map((p) => (p.id === policy.id ? { ...p, allowedRoles: newRoles } : p)));
    await updatePolicyInPb(policy.id, { allowedRoles: newRoles });
  };

  const handleAddUser = async (policy: PolicyRecord, field: "allowedUsers" | "deniedUsers", userId: string) => {
    if (isReadOnly) return;
    const currentList = policy[field] || [];
    if (currentList.includes(userId)) return;
    const newList = [...currentList, userId];
    setPolicies(policies.map((p) => (p.id === policy.id ? { ...p, [field]: newList } : p)));
    await updatePolicyInPb(policy.id, { [field]: newList });
  };

  const handleRemoveUser = async (policy: PolicyRecord, field: "allowedUsers" | "deniedUsers", userId: string) => {
    if (isReadOnly) return;
    const newList = (policy[field] || []).filter((id) => id !== userId);
    setPolicies(policies.map((p) => (p.id === policy.id ? { ...p, [field]: newList } : p)));
    await updatePolicyInPb(policy.id, { [field]: newList });
  };
  ...
  const handleUserOverrideChange = async (
    ...
    const updatedPolicies = policies.map((p) => {
      if (p.id === policy.id) {
        let nextAllowedExpand = [...(p.expand?.allowedUsers || [])].filter(u => u.id !== userId);
        let nextDeniedExpand = [...(p.expand?.deniedUsers || [])].filter(u => u.id !== userId);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          >
            ...
            <div className="relative z-10 mt-4">
              <h4 className="text-sm font-bold text-slate-800 group-hover:text-indigo-600 transition-colors">Role Section Access Policy</h4>
              <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                Manage global feature toggles and Role-Based Access Control (RBAC) rules for each section.
              </p>
            </div>
          </div>
          ...
          >
            ...
            <div className="relative z-10 mt-4">
              <h4 className="text-sm font-bold text-slate-800 group-hover:text-emerald-600 transition-colors">User Access Policy</h4>
              <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                Configure user-wise whitelists and blacklists to grant or deny access to specific operators.
              </p>
            </div>
          </div>
        </div>
  ...
  if (currentView === "role_policy" || currentView === "user_policy") {
    ...
    const titleText = isRoleView ? "Role Section Access Policy" : "User Access Policy";
    const breadcrumbText = isRoleView ? "Settings / Access Control / Roles" : "Settings / Access Control / Users";
      ...
      <div className="space-y-6 animate-fade-in">
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50/60 p-4 text-xs text-amber-900 flex items-start gap-3 shadow-sm">
            ...
            <div className="space-y-1">
              ...
              <p className="text-amber-800 leading-relaxed font-medium">
                You are signed in as a standard administrator. Only **Super Administrators** are permitted to customize dynamic allowed roles and user whitelists/blacklists.
              </p>
            </div>
          </div>
        )}
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center min-h-[300px] gap-2">
            ...
            <span className="text-xs text-slate-400 font-semibold">Loading policies...</span>
          </div>
        ) : (
          <div className="space-y-6">
            {isRoleView 
              ? renderRoleTable(policies.filter(p => p.targetPage === "admin"), policies.filter(p => p.targetPage === "user"))
              : renderUserTable(policies.filter(p => p.targetPage === "admin"), policies.filter(p => p.targetPage === "user"))
            }
          </div>
        )}
      </div>
        ...
        <div
          onClick={() => {
            if (hasUserManagementAccess()) {
              ...
              toast.error("Access to the User Directory is restricted by system policies.");
            }
          }}
          className={`group relative overflow-hidden bg-white border border-blue-100/30 rounded-2xl p-6 shadow-sm hover:shadow-md hover:scale-[1.01] hover:border-blue-100 transition-all duration-300 cursor-pointer flex flex-col justify-between min-h-[160px] ${
            !hasUserManagementAccess() ? "opacity-60 cursor-not-allowed border-slate-100" : ""
          }`}
        >
          ...
          <div className="flex items-start justify-between relative z-10">
            <div className={`p-3 bg-blue-50 rounded-xl transition-all duration-300 ${
              hasUserManagementAccess() ? "text-blue-600 group-hover:bg-blue-600 group-hover:text-white" : "text-slate-400"
            }`}>
            ...
            <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${
              hasUserManagementAccess() ? "text-blue-600 bg-blue-50/50" : "text-slate-400 bg-slate-100"
            }`}>
          ...
          <div className="relative z-10 mt-4">
            ...
            <p className="text-xs text-slate-400 mt-1 leading-relaxed">
              Create, edit, reset passwords, and disable system operators. Manage user roles and directory listings.
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div className="flex items-start justify-between relative z-10">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[9px] font-bold text-indigo-600 bg-indigo-50/50 px-2 py-0.5 rounded-full uppercase tracking-wider">
                {currentUserRole === "super-admin" ? "Configurable" : "Read-Only"}
              </span>
            </div>
          </div>
          ...
          <div className="relative z-10 mt-4">
            ...
            <p className="text-xs text-slate-400 mt-1 leading-relaxed">
              Define Role-Based Access Control (RBAC) and whitelists/blacklists for specific sections of pages.
            </p>
          </div>
        </div>
      </div>
  ...
  function renderRoleTable(adminItems: PolicyRecord[], userItems: PolicyRecord[]) {
    const renderSection = (title: string, items: PolicyRecord[]) => (
      <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm space-y-4">
        <h4 className="font-bold text-slate-800 text-sm border-b border-slate-50 pb-2">{title}</h4>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {items.map((policy) => {
                ...
                return (
                  <tr key={policy.id} className={`hover:bg-slate-50/20 transition-colors ${isSavingThis ? "opacity-75 bg-indigo-50/5" : ""}`}>
                    <td className="py-4 px-2">
                      <div className="font-semibold text-slate-800 flex items-center gap-1.5">
                        {policy.displayName}
                        {isSavingThis && <Loader2 className="h-3 w-3 text-indigo-500 animate-spin" />}
                      </div>
                      <code className="text-[10px] text-slate-400 font-mono">{policy.sectionKey}</code>
                    </td>
                    ...
                    <td className="py-4 px-2">
                      <div className="flex gap-4">
                        {["super-admin", "admin", "student-counsellor", "marketing-manager", "admissions-head"].map((role) => {
                          const checked = policy.allowedRoles.includes(role);
                          return (
                            <label key={role} className={`inline-flex items-center gap-2 font-semibold text-slate-600 ${isReadOnly ? "cursor-not-allowed" : "cursor-pointer"}`}>
                              <input
                                type="checkbox"
                                checked={checked}
                                disabled={isReadOnly}
                              ...
                              />
                              <span className="text-[11px] capitalize">{role.replace("-", " ")}</span>
                            </label>
                          );
                        })}
                      </div>
  ...
  function renderUserTable(adminItems: PolicyRecord[], userItems: PolicyRecord[]) {
    ...
    const renderOverrideControl = (policy: PolicyRecord) => {
      ...
      const currentOverride = isAllowed ? "allowed" : isDenied ? "denied" : "inherited";
      const roleInheritedAccess = policy.enabled && policy.allowedRoles.includes(activeUser?.role || "");
        ...
        <tr key={policy.id} className={`hover:bg-slate-50/20 transition-colors ${isSavingThis ? "opacity-75 bg-indigo-50/5" : ""}`}>
          ...
          <td className="py-3 px-2 text-center">
            <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[9px] font-bold ${
              ...
              {!policy.enabled ? "Disabled Globally" : roleInheritedAccess ? "Allowed" : "Denied"}
            </span>
          </td>
          <td className="py-3 px-2">
            <div className="flex bg-slate-100 p-0.5 rounded-lg w-fit border border-slate-200/40">
              <button
                ...
                onClick={() => handleUserOverrideChange(policy, activeUser!.id, "inherited")}
              ...
              <button
                disabled={isReadOnly || isSavingThis}
                onClick={() => handleUserOverrideChange(policy, activeUser!.id, "allowed")}
                className={`px-3 py-1 rounded-md text-[10px] font-bold transition-all ${
              ...
              <button
                disabled={isReadOnly || isSavingThis}
                onClick={() => handleUserOverrideChange(policy, activeUser!.id, "denied")}
                className={`px-3 py-1 rounded-md text-[10px] font-bold transition-all ${
    ...
    const renderPolicyGroup = (title: string, items: PolicyRecord[]) => (
      <div className="space-y-2">
        <h5 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{title}</h5>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-slate-100 text-slate-400 font-semibold uppercase tracking-wider text-[9px] bg-slate-50/50">
                ...
                <th className="py-2.5 px-2 text-center w-[120px]">Role Default</th>
        ...
        <div className="lg:col-span-2">
          {activeUser ? (
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-50 pb-4">
                <div>
                  <h4 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                    Custom Access Overrides: <span className="text-indigo-600 font-extrabold">{activeUser.name}</span>
                  </h4>
                  <p className="text-[11px] text-slate-400 mt-0.5">
                    Role: <span className="capitalize font-semibold text-slate-600">{activeUser.role.replace("-", " ")}</span> &bull; {activeUser.email}
                  </p>
                </div>
                <div className="bg-slate-50 rounded-xl px-3 py-2 text-[10px] font-semibold text-slate-500 border border-slate-100/50 leading-relaxed max-w-xs">
                  Whitelisted policies allow access regardless of role. Blacklisted policies block access even if the role allows it.
                </div>
              </div>

              <div className="space-y-6">
                {renderPolicyGroup("Admin Dashboard Section Policies", adminItems)}
                {renderPolicyGroup("Allocated User Section Policies", userItems)}
              </div>
            </div>
          ) : (
            <div className="bg-slate-50 border border-slate-100 border-dashed rounded-2xl p-12 text-center flex flex-col items-center justify-center min-h-[300px] gap-3">
              ...
              <h4 className="font-bold text-slate-700 text-sm mt-2">No User Selected</h4>
              <p className="text-xs text-slate-400 max-w-sm leading-relaxed">
                Choose an operator from the left sidebar to customize their individual permissions and whitelist or blacklist them for specific pages.
              </p>
            </div>
          )}
        </div>
